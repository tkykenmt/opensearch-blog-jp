...
公開された記事を `web_fetch` で取得し、以下の観点でレビューする。問題があればローカルファイル `articles/<slug>.md` を修正し、コミット・プッシュを実施する。

#### レビュー観点

**基本方針**: 技術的に正確であれば、より自然な日本語表現を優先する。

1. **画像の表示確認**: リンク切れがないか、alt テキストが適切か
2. **用語の一貫性**: 技術用語が記事内で統一されているか、新しい技術用語に英語併記があるか（初出時）
3. **表記ルール**: 数式が LaTeX 記法で正しくレンダリングされているか、全角・半角間にスペースがあるか、カッコが半角か
4. **日本語の自然さ**: 直訳調で不自然な表現がないか、冗長な表現がないか
5. **避けるべき表現**: 「悪いイベント」→「望ましくないケース」、「ブックキーピング」→「整合性管理」、「0 中心の」→「平均 0 の」
