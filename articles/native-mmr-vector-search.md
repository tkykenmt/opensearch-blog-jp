...
:::message
この記事は [OpenSearch Project の公式ブログ記事](https://opensearch.org/blog/improving-vector-search-diversity-through-native-mmr/) の日本語翻訳です。
:::

https://opensearch.org/blog/improving-vector-search-diversity-through-native-mmr/

検索と推薦システムにおいて、高い関連性を持つ結果を返すことは戦いの半分に過ぎません。同様に重要な要素は多様性です。つまり、ユーザーが複数のほぼ重複した結果ではなく、さまざまな結果を見られるようにすることです。OpenSearch 3.3 は、k-NN およびニューラルクエリに対するネイティブの Maximal Marginal Relevance (MMR) をサポートし、これを簡単にします。
...
MMR は、クエリに関連し、以前に選択された結果とあまり似ていない結果を反復的に選択します。トレードオフは `diversity` パラメータによって制御されます (0 = 関連性を優先、1 = 多様性を優先)。

ベクトル検索では、埋め込みが類似した結果をクラスター化することが多いため、これは特に有用です。MMR がなければ、上位 k 件の結果はすべてほぼ同一に見える可能性があります。
...
すべての上位結果が非常に似ていることに注意してください。ユーザーが見るものにはほとんど多様性がありません。
...
次のパフォーマンス観察は、主な発見を強調しています。

1. MMR はレイテンシを追加し、増加は MMR 候補の数とクエリサイズとともに大きくなります。
2. MMR なしの k-NN およびニューラルクエリは、k の増加を効率的に処理し、計算時間のほとんどは上位 k 候補の選択ではなく、グラフトラバーサル (`ef_search`) に費やされます。

MMR 候補の数を選択するには、多様性とクエリレイテンシのバランスを取る必要があります。候補が多いほど結果の多様性は向上しますが、レイテンシが増加するため、ワークロードに適した値を選択してください。

## クロスクラスター検索での MMR の使用

現在、[クロスクラスター検索](https://docs.opensearch.org/latest/search-plugins/cross-cluster-search/)の場合、OpenSearch はリモートクラスターのインデックスマッピングからベクトルフィールド情報を自動的に解決できません。つまり、MMR を使用する際にベクトルフィールドの詳細を明示的に提供する必要があります。
...
- `candidates` と `diversity`: ローカル MMR クエリと同じで、候補の数と多様性の重みを制御します

この情報を提供することで、リモートクラスター間でクエリを実行する場合でも、MMR が多様性を正しく計算し、結果を再ランキングできるようになります。
...
ベクトル検索の多様性を向上させたい場合、OpenSearch の MMR は今日試すことができる強力なツールです。

## 次は何か?

将来的には、次の更新を提供することで、MMR をさらに使いやすく、より柔軟にする予定です。

- **リモートクラスターのサポート向上**: ベクトルフィールド情報を手動で指定する必要がなくなります。
- **クエリタイプのサポート拡張**: 現在、MMR は `knn_vector` を使用する k-NN クエリまたはニューラルクエリでのみ機能します。`bool` や `hybrid` クエリなどの追加のクエリタイプをサポートすることを目指しており、MMR がより広範な検索シナリオを強化できるようにします。
