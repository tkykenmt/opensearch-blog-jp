...
AWS Lambda の機能の 1 つとして、[レスポンスストリーミング](https://docs.aws.amazon.com/lambda/latest/dg/configuration-response-streaming.html)があります。これにより、関数からクライアントへデータをストリーミングでき、最初のレスポンスのレイテンシーが削減され、最大 200 MB の大きなペイロードもサポートします。
