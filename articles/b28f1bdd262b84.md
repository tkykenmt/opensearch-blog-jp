---
title: "OpenSearch における最新のシャード配置戦略について"
emoji: "👏"
type: "tech"
...
また `cluster.routing.allocation.awareness.force.zone.values` というプロパティも、`cluster.routing.allocation.awareness.attributes` で `zone` を指定することで初めて利用できるようになるわけです。仮に `cluster.routing.allocation.awareness.attributes` で　`rack` と指定していた場合、Forced Awarenss で利用可能なプロパティは `cluster.routing.allocation.awareness.force.rack.values` となります。
...
実際の制限数については、以下の式から求めることができます。ceil は、小数点以下を**切り上げる**関数です。
...
ここに Load Aware Shard Allocation を `skew_factor = 50.0`、`flat_skew = 2` で適用した場合、ノードごとのシャード割り当て数上限は 32 となります。従って、AZ-3 においては、1 ノードに上限の 32 シャードが割り当てられ、残り 8 シャードは未割り当てとなることが期待されます。
